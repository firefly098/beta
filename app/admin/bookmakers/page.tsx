...
export default async function BookmakersListPage({
  searchParams,
}: {
  ...
  return (
    <div>
      <AdminPageHeader
        ...
        actionHref="/admin/bookmakers/new"
        actionLabel="New bookmaker"
      />
      <form className="mb-4">
        <input
          name="q"
            ...
            {items.map((p) => (
              <tr key={p.id} className="border-b border-zinc-100">
                <td className="px-4 py-3">
                  <Link href={`/admin/bookmakers/${p.id}`} className="font-medium hover:underline">
                    {p.name}
                  </Link>
                </td>
