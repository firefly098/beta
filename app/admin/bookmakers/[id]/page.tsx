import { deleteBookmaker, updateBookmaker } from "@/app/admin/actions/content";
import {
  AuthorSelect,
  CtaFields,
  Field,
...
export default async function EditBookmakerPage({
  ...
  const item = await prisma.bookmakerReview.findUnique({ where: { id } });
  if (!item) notFound();
  ...
  const save = updateBookmaker.bind(null, id);
  const remove = deleteBookmaker.bind(null, id);

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-semibold tracking-tight">Edit bookmaker</h1>
        <a
          href={`/bookmakers/${item.slug}`}
          target="_blank"
      ...
      <form action={save} className="space-y-4">
        <div className="grid gap-4 rounded-xl border border-zinc-200 bg-white p-4 sm:grid-cols-2">
          ...
          <Field label="Sports coverage" name="sportsCoverage" defaultValue={item.sportsCoverage} />
          ...
          <Field label="Live betting" name="liveBetting" type="checkbox" defaultValue={item.liveBetting} />
          <Field label="Has apps" name="hasApps" type="checkbox" defaultValue={item.hasApps} />
        </div>
        <div className="grid gap-4 rounded-xl border border-zinc-200 bg-white p-4 sm:grid-cols-2">
