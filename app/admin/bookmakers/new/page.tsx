import { createBookmaker } from "@/app/admin/actions/content";
import {
  AuthorSelect,
  CtaFields,
  Field,
...
export default async function NewBookmakerPage() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-semibold tracking-tight">New bookmaker review</h1>
      <form action={createBookmaker} className="space-y-4">
        <div className="grid gap-4 rounded-xl border border-zinc-200 bg-white p-4 sm:grid-cols-2">
          ...
          <Field label="Sports coverage" name="sportsCoverage" />
          ...
          <Field label="Live betting" name="liveBetting" type="checkbox" defaultValue={true} />
          <Field label="Has apps" name="hasApps" type="checkbox" defaultValue={true} />
        </div>
        <div className="grid gap-4 rounded-xl border border-zinc-200 bg-white p-4 sm:grid-cols-2">
        ...
        <CtaFields defaults={{ ctaLabel: "Visit Bookmaker" }} />
